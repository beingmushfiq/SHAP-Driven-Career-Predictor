...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['analytical_composite'] = (
        df.get('analytical_skills', 0) + 
        df.get('problem_solving_skills', 0) + 
        df.get('coding_skills', 0)
    )
    
    # 2. Communication Composite (communication + presentation + networking)
    df['communication_composite'] = (
        df.get('communication_skills', 0) + 
        df.get('presentation_skills', 0) + 
        df.get('networking_skills', 0)
    )
    
    # 3. Experience Score (internships * 2 + projects + research * 2 + leadership)
    df['experience_score'] = (
        df.get('internships', 0) * 2 +
        df.get('projects', 0) +
        df.get('research_experience', 0) * 2 +
        df.get('leadership_positions', 0)
    )
    ...
    df['cs_coding_interaction'] = np.where(is_cs, df.get('coding_skills', 0), 0.0)
    df['finance_analytical_interaction'] = np.where(is_finance, df.get('analytical_skills', 0), 0.0)
    df['medicine_gpa_interaction'] = np.where(is_medicine, df.get('gpa', 0.0), 0.0)
